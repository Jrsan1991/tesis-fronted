...
export default defineComponent({
  ...
  mounted() {
    this.storeCategory.loadCategories();
    this.getData();
  },
  methods: {
    async getData() {
      const result = await this.store.getProducts();
      if (result.error === 0) {
...
<template>
  ...
  <el-card v-loading="store.loading">
    <div class="flex" v-if="screen.mobile">
      ...
      >
        ...
        <div class="card-header">
          ...
          <p class="data">{{ item.serial }}</p>
        </div>
        <div class="card-header">
          <p class="label">Tipo:</p>
          <p class="data">{{ item.type }}</p>
        </div>
        <div class="card-header">
        ...
        <div class="card-header">
          ...
          <p class="data">{{ item.state }}</p>
        </div>
        <div class="card-header">
          <p class="label">Cantidad:</p>
          <p class="data">{{ item.quantity }}</p>
        </div>
        <div class="card-header">
    ...
    <el-table v-if="!screen.mobile" :data="products" stripe width="100%">
      <el-table-column prop="serial" label="Serie" min-width="100" />
      <el-table-column prop="name" label="Nombre" min-width="150" />
      <el-table-column prop="type" label="Tipo" min-width="100" />
      <el-table-column prop="brand" label="Marca" min-width="100" />
      <el-table-column prop="state" label="Estado" min-width="100" />
      <el-table-column prop="quantity" label="Cantidad" min-width="100" />
      <el-table-column prop="detail" label="Detalle" min-width="150" />
